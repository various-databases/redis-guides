...
public class RedisConsumer implements Closeable {
	...
	public RedisConsumer(int timeout, String groupId, String hostname, int port) {
		...
		while (!jedis.isConnected() && tries < 3) {
			logger.info("----- Retrying redis consumer connection in 3s -----");
			try {
				Thread.sleep(3000);
		...
		logger.info("----- Succesfully connected consumer to {}:{} -----", hostname, port, jedis.isConnected());
	}
	...
	public void close() throws IOException {
		...
		logger.info("Closed consumer with return code of " + rc);
	}
