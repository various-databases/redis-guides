...
public class Nest {
	...
	public Set<Tuple> zrangeByScoreWithScores(double min, double max, int offset, int count) {
		Jedis jedis = getResource();
		Set<Tuple> zrange = jedis.zrangeByScoreWithScores(key(), min, max, offset, count);
		returnResource(jedis);
		return zrange;
	}
